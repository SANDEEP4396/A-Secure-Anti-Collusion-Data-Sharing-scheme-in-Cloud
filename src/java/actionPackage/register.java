...
public class register extends HttpServlet {
     ...
     */
    final String filepath="C:\\Documents and Settings\\Eswar\\Desktop\\";
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(rs.next())
            {
                response.sendRedirect("register.jsp?m=fail");
            }
            else
            {
            int i = st.executeUpdate("insert into register values('"+user+"','"+pass+"','"+group+"','"+email+"','"+mobile+"','"+place+"','NO','0')"); 
             if(i!=0){
                 ...
                 response.sendRedirect("Home.jsp?m1=Registeration done");
                 
             }
             else{
                   response.sendRedirect("register.jsp?m2=register error...check");
             }
            }
            }
            else{
                response.sendRedirect("register.jsp?m3=password not match");
            }
